...
class CustomerKYCRatings  extends ExtractorTrait {
                       ...
                       extractedDF: Option[Map[String, DataFrame]]):Option[Map[String,DataFrame]] =
  {
    ...
    try {
      ...
      logger.info("Customer KYC ratings")

      val inputPath: String = paramsMap(strUtil.INPUT_FILE_PATH).toString  + strUtil.CUSTOMER_KYC_RATINGS
      ...
      Some(Map(strUtil.CUSTOMER_KYC_RATINGS_DF -> samsQuestionTrimDF))
