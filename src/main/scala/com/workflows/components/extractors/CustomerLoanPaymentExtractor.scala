...
class CustomerLoanPaymentExtractor  extends ExtractorTrait {
                       ...
                       extractedDF: Option[Map[String, DataFrame]]):Option[Map[String,DataFrame]] =
  {
    ...
    try {
      ...
      logger.info("Loan or overdrafts repayments (failed/delay to repay)")

      val inputPath: String = paramsMap(strUtil.INPUT_FILE_PATH).toString  + strUtil.CUSTOMER_LOAN_REPAYMENTS
      ...
      Some(Map(strUtil.CUSTOMER_LOAN_REPAYMENTS_DF -> samsQuestionTrimDF))
